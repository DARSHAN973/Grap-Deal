 ...
 * Adapted for Next.js and integrated with existing header design.
 */
const SparkleNavbar = ({ 
  items = [], 
  color = "#00fffc",
  ...
  const navigationItems = items.map((item) => 
    typeof item === 'string' ? { name: item, href: `/${item.toLowerCase()}` } : item
  );

  return (
    <div className={`sparkle-navbar ${className}`}>
      <style jsx>{`
        .sparkle-navbar {
          position: relative;
          z-index: 1;
        ...
        .sparkle-navbar ul li:not(.active):hover button {
          text-shadow: 0 0 10px ${color}, 0 0 20px ${color};
        }

        .sparkle-navbar .active-element {
        ...
        .sparkle-navbar .active-element > svg,
        .sparkle-navbar .active-element .strike {
          position: absolute;
          right: 0;
          top: 50%;
        ...
        .sparkle-navbar .active-element .strike svg {
          display: block;
          overflow: visible;
          height: 12px;
        ...
        .sparkle-navbar .active-element .strike svg:last-child {
          transform: translate(var(--active-element-strike-x), -30%) scale(-1);
        }

        .sparkle-navbar .active-element .strike svg g path:nth-child(2) {
          filter: blur(2px);
        }

        .sparkle-navbar .active-element .strike svg g path:nth-child(3) {
          filter: blur(4px);
        }

        .sparkle-navbar.before .active-element {
          transform: rotateY(180deg);
        }
      `}</style>

      <nav className="navigation-menu text-gray-900 dark:text-gray-100" ref={navRef}>
        <ul>
            ...
            <li key={item.name} className={index === activeIndex ? "active" : ""}>
              <Link href={item.href} passHref>
                <button
                  ...
                  className="text-gray-900 dark:text-gray-100 hover:text-gray-700 dark:hover:text-white transition-colors duration-200"
                >
                  {item.name}
                </button>
              </Link>
